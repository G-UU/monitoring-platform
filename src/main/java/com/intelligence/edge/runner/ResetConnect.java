...
public class ResetConnect implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        /*
        for (CarBasicData uavBasicData : CarTempData.uavList) {
            CarControlServer server = new CarControlServer(uavBasicData.getCarID(), carConfig.getCarControlPort().get(uavBasicData.getCarID()));
            server.openConnect();
            CarTempData.ccsMap.put(server.getCarID(),server);
        }
         */

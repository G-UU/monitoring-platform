...
public class CarNetServiceImpl implements CarNetService {
    ...
    public int connect(String carID) {
        ...
        // 判断是否已经开启
        if (carBasicDataMapper.getConnectState(carID) == 2) {
            log.info(carID+"已处于连接状态");
            return 0;
        } else {
...
//            log.info("修改对象" + param);
            if (carBasicDataMapper.getConnectState(carID) == 1) {
                carBasicDataMapper.setConnectState(carID, 2);
                receiveData(carID);
                receiveVideo(carID);
                return 1;
            }
    ...
    public int closeConnect(String carID) {
        if (carBasicDataMapper.getConnectState(carID) == 0) {
            ...
            param.setCarID(carID);
            if (carBasicDataMapper.getConnectState(carID) == 2) {
                carBasicDataMapper.setConnectState(carID, 1);
    ...
    public void receiveData(String carID) {
        ...
        try {
            ...
            useServer.start();//建立线程处理 打开环境接收端
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

...
/**
 * 环境数据传输Server端
 * 功能说明： 负责接收无人车发送的环境数据（经纬度、速度、温度、湿度...）,存入MySQL数据库中
 */
@Data
        ...
        public void run() {
            ...
            try {
                ...
                while (runFlag) {
                    ...
                    CurrentPostion cpos = new CurrentPostion(carID,ei.getLongitude(),ei.getLatitude());

                    // websockt 推送
                    String jsonString = JSONObject.toJSONString(cpos);
